...
# K8S Incident Triage Assistant

{: .no_toc }

<span class="label label-blue">{{ page.subcategory }}</span>

The **Incident Triage Assistant** plugin provides an automated diagnostic agent that investigates Kubernetes workload failures on-demand, triggered by Alertmanager webhooks or manual developer actions. When a pod is `OOMKilled`, an image fails to pull, or a container enters a crash loop, the assistant collects bounded cluster diagnostics, classifies the failure signature, and generates a cited, evidence-backed triage report detailing likely root causes and next steps.

To ensure strict production safety, the assistant is **purely read-only and diagnostic**. It leverages a secure cluster driver contract to aggregate logs, events, and container states, but it lacks any mutation capabilities - meaning it can never execute restarts, rollbacks, or scaling actions.

## Key Features

- **Deterministic Failure Classification**: Maps errors (`OOMKilled`, `ImagePullBackOff`, `CrashLoopBackOff`, `ProgressDeadlineExceeded`) directly to tailored evidence-collection plans without relying on model-driven discovery.
- **Bounded Log Collection**: Safeguards system memory by strictly capping log collection for terminated or restarting containers using `maxLogBytes` and `maxLogContainers` limits.
- **Citation-Backed Diagnostics**: Generates a report separating observed cluster data from model inference, explicitly tracking claims back to specific evidence IDs.
- **Fail-Safe Fallbacks**: Reverts automatically to a pre-defined, deterministic list of failure-class causes if the model fails validation or leaves claims uncited.
- **Automated Data Redaction**: Secures sensitive workloads by stripping bearer tokens, AWS keys, PEM private keys, and credentials from all evidence items prior to prompting.
- **Multi-Turn Incident Memory**: Maintains a persistent conversation session allowing operators to ask iterative, follow-up troubleshooting questions about the active failure.
- **Dual-Trigger Architecture**: Supports both automated Alertmanager webhook dispatches and manual execution via an Entity Page action button and a dedicated UI triage page.
